...
IF NOT EXISTS (SELECT 1 FROM msdb.dbo.sysalerts WHERE name='017- Insufficient Resources')   
	BEGIN    
		EXEC msdb.dbo.sp_add_alert @name=N'017- Insufficient Resources',     @message_id=0    ,     @severity=17   ,     @enabled=1,     @delay_between_responses=600,     @include_event_description_in=2,     @job_id=N'00000000-0000-0000-0000-000000000000'   
	END
	GO   
	IF NOT EXISTS( SELECT 1 FROM msdb.dbo.sysnotifications sy        INNER JOIN msdb.dbo.sysalerts sa ON sy.alert_id = sa.id        INNER JOIN msdb.dbo.sysoperators so ON sy.operator_id = so.id       WHERE so.name = 'SQLAdmins' AND        sa.name = '017- Insufficient Resources')   BEGIN    EXEC msdb.dbo.sp_add_notification @alert_name=N'017- Insufficient Resources', @operator_name=N'SQLAdmins', @notification_method = 1;   
	END  
	GO